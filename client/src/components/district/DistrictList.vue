  ...
  confirm.require({
    ...
    accept: async () => {
      try{
        ...
        toast.add({ severity: 'success', summary: 'Success', detail: 'District Deleted!', life: 3000 });
      } catch (error) {
...
<template>
  ...
  <Panel header="Manage Districts">
    <DataTable ref="dt" :value="districts" stripedRows sortField="usd" :sortOrder="1" tableStyle="min-width: 50rem" :exportFunction="exportFunction">
      <template #header>
        <Toolbar class="mb-2" style="border: none;">
          <template #start>
            <Button label="New" icon="pi pi-plus" severity="success" class="mr-2" @click="newDistrict" />
          </template>
          <template #end>
      ...
      <Column field="teachers" header="Teachers">
        <template #body="slotProps">
          <Tag v-for="teacher in slotProps.data.teachers" :key="teacher.id" :value="teacher.name" severity="secondary"/> 
        </template>
      </Column>
      <Column header="Actions" :exportable="false" style="min-width:8rem">
          <template #body="slotProps">
              <Button icon="pi pi-pencil" outlined rounded class="mr-2" @click="editDistrict(slotProps.data)" v-tooltip.bottom="'Edit'"/>
              <Button icon="pi pi-trash" outlined rounded severity="danger" @click="deleteDistrict(slotProps.data)" v-tooltip.bottom="'Delete'"/>
          </template>
      </Column>
    </DataTable>
  </Panel>

  <Dialog v-model:visible="districtDialog" :style="{width: '450px'}" :header="districtDialogHeader" :modal="true" class="p-fluid" :closeOnEscape="true">
    ...
    <div class="flex flex-column align-items-center row-gap-5 w-full pt-3 mt-1" v-focustrap v-on:keyup.enter="save">
      <TextField :model="district" field="name" label="Name" icon="pi pi-user" :errors="errors"/>
      <TextField :model="district" field="usd" label="USD" icon="pi pi-key" :errors="errors"/>
      <TextField :model="district" field="url" label="URL" icon="pi pi-at" :errors="errors"/>
      <AutocompleteMultiple :model="district" field="teachers" label="Teachers" icon="pi pi-users" :errors="errors" :values="teachers" valueLabel="name"/>
