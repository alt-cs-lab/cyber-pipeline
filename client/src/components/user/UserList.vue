...
<template>
  ...
  <Panel header="Manage Users">
    <DataTable ref="dt" :value="users" stripedRows sortField="eid" :sortOrder="1" tableStyle="min-width: 50rem" :exportFunction="exportFunction">
      <template #header>
        <Toolbar class="mb-2" style="border: none;">
          <template #start>
            <Button label="New" icon="pi pi-plus" severity="success" class="mr-2" @click="newUser" />
          </template>
          <template #end>
      ...
      <Column field="roles" header="Roles">
        <template #body="slotProps">
          <Tag v-for="role in slotProps.data.roles" :key="role.id" :value="role.name" :icon="role.name == 'admin' ? 'pi pi-key' : ''" :severity="role.name == 'admin' ? 'danger' : 'secondary'"/> 
        </template>
      </Column>
      <Column header="Actions" :exportable="false" style="min-width:8rem">
          <template #body="slotProps">
              <Button icon="pi pi-pencil" outlined rounded class="mr-2" @click="editUser(slotProps.data)" v-tooltip.bottom="'Edit'"/>
              <Button icon="pi pi-trash" outlined rounded severity="danger" @click="deleteUser(slotProps.data)" v-tooltip.bottom="'Delete'"/>
          </template>
      </Column>
    </DataTable>
  </Panel>

  <Dialog v-model:visible="userDialog" :style="{width: '450px'}" :header="userDialogHeader" :modal="true" class="p-fluid" :closeOnEscape="true">
    ...
    <div class="flex flex-column align-items-center row-gap-5 w-full pt-3 mt-1" v-focustrap v-on:keyup.enter="save">
      <TextField :model="user" field="eid" label="eID" icon="pi pi-at" :errors="errors" :disabled="!editEid"/>
      <TextField :model="user" field="name" label="Name" icon="pi pi-user" :errors="errors"/>
      <AutocompleteMultiple :model="user" field="roles" label="Roles" icon="pi pi-check-square" :errors="errors" :values="roles" valueLabel="name"/>
