import path$2, { win32 } from 'node:path';
...
function read(jsonPath, {base, specifier}) {
  ...
  try {
    string = fs.readFileSync(path$2.toNamespacedPath(jsonPath), 'utf8');
  } catch (error) {
...
function emitLegacyIndexDeprecation(url, packageJsonUrl, base, main) {
  ...
  } else if (path$2.resolve(packagePath, main) !== urlPath) {
    process$1.emitWarning(
...
function finalizeResolution(resolved, base, preserveSymlinks) {
...

  {
    ...
    resolved = pathToFileURL$1(real + (filePath.endsWith(path$2.sep) ? '/' : ''));
